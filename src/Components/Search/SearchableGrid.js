...
import Subordinatelist from '../Subordinates/Subordinatelist';
...
export default class SearchableGrid extends Component {
	...
	render(){
		return (
			...
			<Router>
			<Switch>
			<Route path="/Subordinatelist" component={Subordinatelist} />
			<table>
				<tbody>
					<th>Sr. No</th>
					<th>Title</th>
				...
				</tbody>
				{
					this.state.datas.filter(this.searchingFor(this.props.searchStr)).map((data, index) => 
						<tr>
							<td>{index + 1}</td>
							<td>{data}</td>
							<td><Link to={`/Subordinatelist?q=${data}`}>View Subordinates</Link></td>
						</tr>
					)}
				</table>
				</Switch>
